...
var jqMock = (function ($) {
	...
	function _getClassName(clz) {
		try {
		    ...
		    if ((clz instanceof Error) && clz.name)
		        return clz.name;
		    else
			    return(clz.toString());
		}
	...
	function expectThatExceptionThrown(fn, expression) {
		try {
			fn();
			jqUnit.ok(false, "The expected exception was never thrown. Expression was: " + tostring(expression)); 
		} catch(ex) {
	...
	var ExpressionFactory = {
		...
		exception: function(param) {
			...
			if ("type" in param && (typeof param["type"] != "function"))
